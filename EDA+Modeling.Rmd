...
We remove meaningless features and incomplete cases. As I will need the Id column later, I haven't removed it at this point.
```{r NA transformation}
...
What we do need to make now is to apply the log to those whose skewness value is below a given threshold that we've set in 25. So only a few variables are affected: 
```{r}
skew
```


But to be honest - I could do no skewness (except from SalePrice) and still get the same result. That implies that the features are not as skewed as SalePrice. The lower I set the threshold the higher RMSE:
threshold = 25 --> Kaggle RMSE: 0.12490
threshold = 10 --> Kaggle RMSE: 0.12290
...
But again.. ended in higher RMSE
```{r}
...
Experiencing the same problem as with polynomial feature creation while doing dummifying of **all** categorical variables (resulting in a Kaggle score of 0.12530), I decided to encode only specific categoric variables:
```{r}
...
This part is equivalent to the Chi Squared, but with another metric. This model produces lots of 0 values in correlation within the df infogain - therefore the threshold is 0.083 (= 3rd quartile) hard coded.

```{r IG Selection}
infogain <- information.gain(SalePrice ~., training)
training_without_SalePrice <- training[,-which(names(training) == "SalePrice")]
infogain <- cbind(infogain, "features"= colnames(training_without_SalePrice))
infogain<-infogain[order(-infogain$attr_importance),]
infogain_plot <- infogain[1:15,]

# Plot the result, and remove those below the 1st IQR (inter-quartile-range) --aggressive
par(mfrow=c(1,2))
bp.stats <- boxplot.stats(abs(infogain$attr_importance))$stats   # Get the statistics from the boxplot 
boxplot(abs(infogain$attr_importance)) # + text(y = bp.stats,  labels = sapply(bp.stats, function(x){format(round(x, 3), nsmall=3)}),  x = 1.3, cex=0.7))# This is to reduce the nr of decimals

infogain.threshold4 = bp.stats[4]
infogain.threshold3 = bp.stats[3]  # This element represent the median.
barplot(sort(infogain_plot$attr_importance), names.arg = infogain_plot$features, cex.names = 0.6, las=2, horiz = T) + abline(v=infogain.threshold4, col='red')  # Draw a red line over threshold
```

Now, we can test if this a good move, by removing any feature with a Information Gain test statistic against the output below the 1 IQR.

```{r message=FALSE, warning=FALSE}
# Determine what features to remove from the training set.
features_to_remove <- as.character(infogain[infogain$attr_importance < infogain.threshold3, "features"])
lm_IG_removed <-lm.model(training[!names(training) %in% features_to_remove], validation, "Infogain Model")
comp <- rbind(comp, data.table(model = "lm_IG_removed", eval = lm_IG_removed$labels$title$title, threshold = infogain.threshold3));
features_to_remove <- as.character(infogain[infogain$attr_importance < infogain.threshold4, "features"])
lm_IG_removed <-lm.model(training[!names(training) %in% features_to_remove], validation, "Infogain Model")
comp <- rbind(comp, data.table(model = "lm_IG_removed", eval = lm_IG_removed$labels$title$title, threshold = infogain.threshold4));
datatable((comp),rownames = FALSE, options = list(lengthChange = FALSE,searching = FALSE, info= FALSE, paginate = FALSE))
```
...
As I am a bit frustrated that everything I tried increased my RMSE and as well often confused what is right and wrong, I'd be super happy if you could upload your code in the end, to show what would have been the best approach. Thank you very much!  
